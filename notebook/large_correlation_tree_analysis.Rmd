...
dendro = ggraph(gr, layout = 'dendrogram', circular = TRUE) + 
    ...
    scale_radius() +
    #viridis::scale_color_viridis() +
    coord_fixed() + theme_void() + scale_alpha("Diet associations with\ntaxonomic tree nodes\n(R2, PERMANOVA)", labels = scales::percent) + scale_size("Diet associations with\ntaxonomic tree nodes\n(R2, PERMANOVA)", labels = scales::percent)
...
infraspecies %>%
  ...
  ggplot() + 
  geom_point(aes(x=value+10^-4,y=meat_plant_ratio)) + 
...
Erectale$binary_msp %>% 
  ...
  ylab("Genes") +
  xlab(substitute(paste("Metagenomic ", italic('E. rectale'), " infraspecies"))) -> p_erectale_infraspecies
...
cowplot::plot_grid(
  ...
  cowplot::plot_grid(p_erectale_infraspecies,p_cazotypes_read_mass,nrow=2, labels = c("B","D")), 
  ncol=2, rel_widths = c(2,1))

ggsave("Figure2_infraspecies_diet_cazotypes.pdf")
