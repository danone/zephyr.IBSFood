...
R2_permanova_diet_cazy_read_mass_df %>% 
  filter(R2_class=="(0.02,1]") %>%
  with(.,pairwise.wilcox.test(read_mass , enterotypes %>% as.character, data=.))
...
MSP_CAZY_df %>%
  ...
  summarize(n=n()) %>%
  filter(cazy.family %in%  c("GH2","GH13","GH43","GH20","GH92","CBM32","CBM26") ) %>%
  ...
  ggplot() + geom_point(aes(x=R2,y=CBM32))
