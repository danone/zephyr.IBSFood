...
```{r}
 ...
 cor(microbial_otu_pco$tab[,"A3"],unifrac_pco$tab[,"A1"])
 


 cor(microbial_otu_pco$tab[,"A3"],t(genus), method="spearman")  %>% t %>% as.data.frame %>% tibble::rownames_to_column() %>% arrange(desc(abs(V1)))
 
 
 cor(unifrac_pco$tab[,"A1"],t(OTU)[colnames(genus),], method="spearman")  %>% t %>% as.data.frame %>% tibble::rownames_to_column() %>% arrange(desc(abs(V1)))
 

 
 cor(microbial_otu_pco$tab[,"A1"],unifrac_pco$tab[,"A10"])
 
 
  cor(microbial_otu_pco$tab[,"A1"],t(genus), method="spearman")  %>% t %>% as.data.frame %>% tibble::rownames_to_column() %>% arrange(desc(abs(V1)))
 
 
 cor(unifrac_pco$tab[,"A10"],t(OTU)[colnames(genus),], method="spearman")  %>% t %>% as.data.frame %>% tibble::rownames_to_column() %>% arrange(desc(abs(V1)))
