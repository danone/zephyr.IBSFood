...
cor(infraspecies_food_coi$lX,infraspecies_pco$li[1:5]) %>%
  reshape2::melt() %>%
  group_by(Var2) %>%
  top_n(2)
...
food_coi_cor %>%
  ...
  filter(abs(A1_cor)>0.3|abs(A2_cor)>0.3|abs(A3_cor)>0.3) %>% arrange(A2_cor) %>%
...
food_coi_cor %>%
  ...
  ggplot() + ggrepel::geom_label_repel(aes(x=A1_cor, y=A3_cor, label=`Food groups lvl3`), size=2, label.size = NA, fill=NA)
...
select(A1_cor,A2_cor,A3_cor,nutriscore,cor_A1,cor_A2,cor_A3) %>%
