...
infraspecies_pco = dudi.pco(A %>% ade4::quasieuclid(), scannf = F, nf=79)

food_pco = dudi.pco(B %>% ade4::quasieuclid(), scannf = F, nf=79)
...
coinertia(dudi.pca(infraspecies_pco$li, scannf=F, nf=79),dudi.pca(food_pco$li, scannf=F, nf=79), scannf=F, nf=3) %>% randtest
...
infraspecies_pco = ade4::dudi.pco(A , scannf=F, nf=78)$li

food_pco = ade4::dudi.pco(B %>% ade4::quasieuclid(), scannf=F, nf=78)$li

infraspecies_food_procrustes = vegan::procrustes(infraspecies_pco,food_pco, symmetric = T)

(procrust_test_p <- vegan::protest(infraspecies_pco, food_pco, permutations = how(nperm = 999))$signif)


## permute

r = sample(1:79, replace=F)


A_r = A %>% as.matrix %>% .[r,r]


row.names(A_r) = colnames(A_r) = row.names(food_pco)


infraspecies_pco_r = ade4::dudi.pco(A_r %>% as.dist() , scannf=F, nf=78)$li

#infraspecies_pco_r = infraspecies_pco_r[r,]
#row.names(infraspecies_pco_r) = row.names(food_pco)


infraspecies_food_procrustes_r = vegan::procrustes(infraspecies_pco_r,food_pco, symmetric = T)

(procrust_test_p <- vegan::protest(infraspecies_pco_r, food_pco, permutations = how(nperm = 999))$signif)
...
for(i in 1:79) { d[i] = dist(rbind(infraspecies_food_procrustes$X[i,], infraspecies_food_procrustes$Yrot[i,]))  }

for(i in 1:79) { d_random[i] = dist(rbind(infraspecies_food_procrustes_r$X[i,], infraspecies_food_procrustes_r$Yrot[i,]))  }



data.frame(d=c(d,d_random), test = c(rep("observed",79), rep("simulated",79))    ) %>%
  ggplot() + geom_boxplot(aes(y=d,x=test)) + xlab("euclidean distance\nbetween food and microbiota") + theme_classic()

wilcox.test(d,d_random)
...
coinertia(dudi.pca(phylum_pco$li, scannf=F, nf=78),dudi.pca(food_pco$li, scannf=F, nf=78), scannf=F, nf=3) %>% randtest
...
coinertia(dudi.pca(genus_pco$li, scannf=F, nf=78),dudi.pca(food_pco$li, scannf=F, nf=78), scannf=F, nf=3) %>% randtest
...
infraspecies_pco = dudi.pco(A %>% ade4::quasieuclid(), scannf = F, nf=78)

food_pco = dudi.pco(B %>% ade4::quasieuclid(), scannf = F, nf=78)


infraspecies_food_coi = coinertia(infraspecies_pco,food_pco, scannf=F, nf=10)

coinertia(dudi.pca(infraspecies_pco$li, scannf=F, nf=78),dudi.pca(food_pco$li, scannf=F, nf=78), scannf=F, nf=3) %>% randtest
...
infraspecies_food_coi = coinertia(infraspecies_pco,food_pco, scannf=F, nf=18)
...
coinertia(dudi.pca(infraspecies_pco$li, scannf=F, nf=78),dudi.pca(food_pco$li, scannf=F, nf=78), scannf=F, nf=10) %>% randtest
...
##### compare euclidean distance to random
...
for(i in 1:79) { d[i] = euc.dist(infraspecies_food_coi$mY[i,], infraspecies_food_coi$mX[i,])  }

for(j in 1:10) {

r = sample(1:79, replace=F)

infraspecies_pco_r = dudi.pco(A %>% ade4::quasieuclid() %>% as.matrix %>% .[r,r] %>% as.dist  , scannf = F, nf=78)

food_pco_r = dudi.pco(B %>%ade4::quasieuclid() , scannf = F, nf=78)


infraspecies_food_coi_r = coinertia(infraspecies_pco_r,food_pco_r, scannf=F, nf=18)


for(i in 1:79) { 
  
  
  d_random[i] = euc.dist(infraspecies_food_coi_r$mY[i,], infraspecies_food_coi_r$mX[i,])  
  
  
}

d_random_all = c(d_random_all, d_random)

}



data.frame(d=c(d,d_random_all), test = c(rep("observed",79), rep("simulated",790))    ) %>%
  ggplot() + geom_boxplot(aes(y=d,x=test)) + xlab("euclidean distance\nbetween food and microbiota") + theme_classic()


wilcox.test(d~test, data=data.frame(d=c(d,d_random_all), test = c(rep("observed",79), rep("simulated",790))    ))
...
save(food_otu, file="food_otu.rda")
...
### Permanova with each food and nutrient
...
infraspecies_food_coi$aX %>% tibble::rownames_to_column("pco")   %>% melt %>% filter(abs(value)>0.5)
infraspecies_food_coi$aY %>% tibble::rownames_to_column("pco")   %>% melt %>% filter(abs(value)>0.5)
...
save(infraspecies_food_coi,file = "infraspecies_food_coi.rda")
save(infraspecies_food_coi_spesup, file="infraspecies_food_coi_spesup.rda")
save(infraspecies_food_coi_foodsup, file="infraspecies_food_coi_foodsup.rda")
...
rbind(infraspecies_food_coi$lX,infraspecies_food_coi_spesup$lisup)[1:5] %>%
  merge(.,IBSMicrobiota::IBSData$metadata %>%
          filter(Visit=="V4",Sample_type=="Stool"), by.x="row.names", by.y="Patient_ID") %>%
  mutate(breath_H2 = ifelse(H2>10,"High_H2","Low_H2"), breath_CH4 = ifelse(CH4>10,"High_CH4","Low_CH4")) %>%
  ggplot() + geom_boxplot(aes( y=AxcX5, x=paste(breath_H2,breath_CH4))) + coord_flip()  
...
rbind(infraspecies_food_coi$lX,infraspecies_food_coi_spesup$lisup)[1:5] %>%
  merge(.,IBSMicrobiota::IBSData$metadata %>%
          filter(Visit=="V4",Sample_type=="Stool"), by.x="row.names", by.y="Patient_ID") %>%
  mutate(breath_H2 = ifelse(H2>10,"High_H2","Low_H2"), breath_CH4 = ifelse(CH4>10,"High_CH4","Low_CH4")) %>%
  ggplot() + geom_boxplot(aes( y=AxcX4, x=paste(breath_H2,breath_CH4))) + coord_flip()  
...
# model cross validation using DAAG::cvlm

train = microbiota_data_gas %>%
  filter(group=="train") %>%
  select(AxcX1,AxcX2,AxcX3,AxcX4,AxcX5,H2_CH4) #%>%
  #mutate(H2_CH4 =  scale(H2_CH4))

microbiota_model_cv = DAAG::CVlm(data=train %>% na.omit , form.lm = H2_CH4~., m=5 )

cor.test(microbiota_model_cv$Predicted, microbiota_model_cv$H2_CH4, method="spearman")

plot(microbiota_model_cv$cvpred, microbiota_model_cv$Predicted, xlim=c(-3,3), ylim=c(-3,3))
abline(b=1,a=0)
...
ggsave("food_based_microbiota_H2_CH4_plot.pdf")
...
> TO DO: predict severity with logistics regression based on Principal Component


## visualisations

### infraspecies co-inertia
...
food_coi_cor %>%
  ...
  unique() %>%
  filter(abs(A1_cor)>0.25|abs(A2_cor)>0.25)  %>%
  ggplot() + ggrepel::geom_label_repel(aes(x=A1_cor, y=A2_cor, label=`Food groups lvl3`), size=2, label.size = NA, fill=NA) + ggrepel::geom_label_repel(aes(x=A1_cor, y=A2_cor, label=`Var1`), size=2, label.size = NA, fill=NA) +
  xlab("association with Coi1") + ylab("association with Coi2")
...
food_coi_cor %>%
  ...
  unique() %>%
  filter(abs(A1_cor)>0.25|abs(A2_cor)>0.25)  %>%
  ggplot() + ggrepel::geom_label_repel(aes(x=A1_cor, y=A2_cor, label=`Var1`), size=2, label.size = NA, fill=NA) +
  xlab("association with Coi1") + ylab("association with Coi2")
...
food_coi_cor %>%
  ...
  ggplot() + 
  ggrepel::geom_label_repel(aes(x=A1_cor, y=A4_cor, label=`Food groups lvl3`, color=nutriscore), size=2, label.size = NA, fill=NA) + 
  ggrepel::geom_label_repel(aes(x=A1_cor, y=A4_cor, label=`Var1`), size=2, label.size = NA, fill=NA) + 
...
food_coi_cor %>%
  ...
  #arrange(`Food groups lvl4`), by.x="Var1", by.y="Food groups lvl4") %>%
  unique() %>%
  filter(abs(A2_cor)>0.3|abs(A4_cor)>0.3) %>% arrange(A4_cor) %>%
  ggplot() + ggrepel::geom_label_repel(aes(x=A2_cor, y=A4_cor, label=Var1, color=nutriscore), size=2, label.size = NA, fill=NA) +
  ggrepel::geom_label_repel(aes(x=A2_cor, y=A4_cor, label=`Food groups lvl3`, color=nutriscore), size=2, label.size = NA, fill=NA) +
  xlab("association with Coi2") + ylab("association with Coi4")

ggsave("food_association_with_coinertia.pdf")
...
select(A1_cor,A2_cor,A3_cor,A5_cor,nutriscore,cor_A1,cor_A2) %>%
  ggplot() + geom_point(aes(x=nutriscore, y=A2_cor, col=nutriscore)) + ylab("Coinertia PC2")

food_coi_cor %>%
merge(.,nutriscore_food_group, by.y="Food groups lvl4", by.x="Var1") %>%
select(A1_cor,A2_cor,A3_cor,A4_cor,A5_cor,nutriscore,cor_A1,cor_A2) %>%
  ggplot() + geom_point(aes(x=nutriscore, y=A4_cor, col=nutriscore)) + ylab("Coinertia PC4")
...
food_coi_cor %>%
  ...
  filter(abs(A1_cor)>0.3|abs(A2_cor)>0.3|abs(A4_cor)>0.3) %>% arrange(A4_cor) %>%
  ggplot() + 
  ggrepel::geom_label_repel(aes(x=A1_cor, y=A2_cor, label=`Food groups lvl3`, color=nutriscore), size=2, label.size = NA, fill=NA) + 
  ggrepel::geom_label_repel(aes(x=A1_cor, y=A2_cor, label=`Var1`), size=2, label.size = NA, fill=NA) + 
  xlab("association with Coi1") + ylab("association with Coi2")
...
food_coi_cor %>%
  ...
  filter(abs(A1_cor)>0.3|abs(A2_cor)>0.3|abs(A4_cor)>0.3) %>% arrange(A4_cor) %>%
  ggplot() + 
  geom_point(aes(x=A2_cor, y=A4_cor)) +
  ggrepel::geom_label_repel(aes(x=A2_cor, y=A4_cor, label=`Food groups lvl3`, color=nutriscore), size=2, label.size = NA, fill=NA) + 
  ggrepel::geom_label_repel(aes(x=A2_cor, y=A4_cor, label=`Var1`), size=2, label.size = NA, fill=NA) + 
  geom_hline(yintercept = c(-0.3,0.3)) + geom_vline(xintercept = c(-0.3,0.3)) +
  xlab("association with Coi2") + ylab("association with Coi4")

ggsave("co_inertia_food_ax2_ax4.pdf")
...
```{r}
food_group_levels %>% filter(`Food groups lvl4` == "28_2")
...
merge(meat_plant_ratio, by.x="variable",
  ...
  filter(Visit=="V4") %>%
  ggplot() + geom_boxplot(aes(x=enterotypes %>% as.character,y=Alcohol))
...
merge(meat_plant_ratio, by.x="variable",
  enterotypes, by.y="Patient_ID") %>%
  filter(Visit=="V4") %>%
  select(-Visit,-Sample_ID,-variable) %>%
  #select(meat_plant_ratio, enterotypes) %>%
  reshape2::melt(id.vars="enterotypes") %>%
  group_by(variable) %>%
  do(w = with(.,pairwise.wilcox.test(value,enterotypes, p.adjust="none"))) %>% broom::tidy(w)
...
merge(meat_plant_ratio, by.x="variable",
  ...
  enterotypes, by.y="Patient_ID") %>%
  filter(Visit=="V4") %>%
  ggplot() + geom_boxplot(aes(x=as.character(cazotype),y=Alcohol))
...
merge(meat_plant_ratio, by.x="variable",
  ...
  enterotypes, by.y="Patient_ID") %>%
  filter(Visit=="V4") %>%
  select(-Visit,-Sample_ID,-variable, -enterotypes) %>%
  #select(meat_plant_ratio, enterotypes) %>%
  reshape2::melt(id.vars="cazotype") %>%
  group_by(variable) %>%
  do(w = with(.,pairwise.wilcox.test(value,cazotype, p.adjust="none"))) %>% broom::tidy(w) %>% mutate(p.value=round(p.value,3))
...
enterotypes %>%
  ...
  arrange(enterotypes) %>%
  ggplot() + 
  geom_tile(aes(x = rep(" ",length(enterotypes)), y=1:length(enterotypes),  fill=enterotypes %>% as.character)) + 
  scale_fill_brewer("Enterotypes",type="qual", palette = "Set1") +
...
enterotypes %>%
  ...
  scale_fill_brewer("CAZotypes",type="qual", palette = "Set2") +
  #geom_tile(aes(x = rep("Visit",length(enterotypes)), y=1:length(enterotypes),  fill=Visit %>% as.character)) +
  geom_tile(aes(x = rep(" ",length(enterotypes)), y=1:length(enterotypes),  fill=cazotype %>% as.character)) +
  coord_flip() + theme_minimal() + xlab("") + ylab(""), #+ guides(fill=TRUE),
...
```{r, eval=FALSE, fig.height=8, fig.width=4, message=FALSE, warning=FALSE, include=FALSE}
...
unifrac_all_pco  = ade4::dudi.pco(food_unifrac %>% ade4::quasieuclid(), scannf=F, nf=5)
infraspecies_all_pco = dudi.pco(infraspecies_jsd, scannf = F, nf=10)

select_sample_mcoa = intersect(infraspecies_all_pco$li %>% row.names, unifrac_all_pco$li %>% row.names)


IBSMicrobiota::IBSData$metadata %>% 
  filter(Visit == "V4", Sample_type =="Stool") %>%
  select(Patient_ID, Health, Age, BMI, H2, CH4, IBS.Severity.Score, HAD.ANXIETY, HAD.DEPRES, BSF.Mean_Stoolform_total, BSF.Mean_Frequenc_StoolPerDay, transit.days) %>%
  mutate(IBS.Severity.Score = ifelse(Health=="control", 0,  IBS.Severity.Score)) %>%
  select(-Health) %>%
  melt(id.vars="Patient_ID") %>%
  group_by(variable) %>%
  mutate(value = ifelse(is.na(value), median(value %>% na.omit), value)) %>%
  dcast(Patient_ID~variable) %>%
  merge(infraspecies_all_pco$li, by.x="Patient_ID", by.y="row.names") %>%
  tibble::column_to_rownames("Patient_ID") %>% 
  cor(method="spearman") %>%
  melt() %>%
  filter(abs(value)>0.2 & value != 1)
...
IBSMicrobiota::IBSData$metadata %>% 
  ...
  mutate(value = ifelse(is.na(value), median(value %>% na.omit), value)) %>%
  dcast(Patient_ID~variable) %>%
  merge(unifrac_all_pco$li, by.x="Patient_ID", by.y="row.names") %>%
  tibble::column_to_rownames("Patient_ID") %>% 
  cor(method="spearman") %>%
  melt() %>%
  filter(abs(value)>0.2 & value != 1)
...
IBSMicrobiota::IBSData$metadata %>% 
  filter(Visit == "V4", Sample_type =="Stool") %>%
  filter(Patient_ID %in% select_sample_mcoa) %>%
  select(Patient_ID, Health, Age, BMI, H2, CH4, IBS.Severity.Score, HAD.ANXIETY, HAD.DEPRES, BSF.Mean_Stoolform_total, BSF.Mean_Frequenc_StoolPerDay, transit.days) %>%
  mutate(IBS.Severity.Score = ifelse(Health=="control", 0,  IBS.Severity.Score)) %>%
  select(-Health) %>%
  melt(id.vars="Patient_ID") %>%
  ...
  tibble::column_to_rownames("Patient_ID") %>%
  .[select_sample_mcoa,] %>%
  merge(.,nutriscore_DI, by.y="Patient_ID", by.x="row.names") %>%
...
metadata_79ind %>% 
  scale(scale=TRUE) %>%
  as.data.frame %>%
  select(CH4,H2,IBS.Severity.Score, transit.days, nutriscore_DI)


w.ktab = cbind(infraspecies_all_pco$li[select_sample_mcoa,1:5],unifrac_all_pco$li[select_sample_mcoa,1:5], metadata_79ind[select_sample_mcoa,] %>% 
  scale(scale=TRUE) %>%
  as.data.frame %>%
  select(CH4,H2,IBS.Severity.Score, BSF.Mean_Stoolform_total, nutriscore_DI)) %>%
  ktab.data.frame(blocks = c(5,5,5), tabnames = c("microbiota","food","metadata"))


microbiota_food_mcoa <- mcoa(w.ktab,  scan = FALSE, nf = 5)

microbiota_food_mcoa
summary(microbiota_food_mcoa)
plot(microbiota_food_mcoa)


s.corcircle(microbiota_food_mcoa$axis  , clabel = 0.5, yax = 2)

s.corcircle(microbiota_food_mcoa$axis, clabel = 0.5, yax = 3)

s.corcircle(microbiota_food_mcoa$axis, clabel = 0.5, yax = 4)

s.corcircle(microbiota_food_mcoa$axis, clabel = 0.5, yax = 5)

cumsum(microbiota_food_mcoa$pseudoeig)/sum(microbiota_food_mcoa$pseudoeig)


microbiota_food_mcoa$Tli
...
sample_sup <- as.matrix((infraspecies_all_pco$li[,1:5])) %*% (as.matrix(microbiota_food_mcoa$axis[1:5,]))

sample_sup[c("M172","M097"),]


as.matrix(microbiota_food_mcoa$Tli[c("M172.microbiota","M097.microbiota"),])


unclass(microbiota_food_mcoa)

microbiota_food_mcoa$Tco
...
data.frame(microbiota_food_mcoa$SynVar,metadata_79ind, unifrac_all_pco$li[select_sample_mcoa,1:5], infraspecies_all_pco$li[select_sample_mcoa,1:5]) %>%
  select(SynVar2,A2,A1.1,nutriscore_DI) %>%
  melt(id.vars="SynVar2") %>%
  ggplot(aes(x=SynVar2, y=value)) + geom_point() + geom_smooth(method = "lm") + facet_wrap(~variable, scale="free_y")
...
data.frame(microbiota_food_mcoa$SynVar,metadata_79ind, unifrac_all_pco$li[select_sample_mcoa,1:5], infraspecies_all_pco$li[select_sample_mcoa,1:5])  %>%
  ...
  melt(id.vars="SynVar2") %>% 
  group_by(variable) %>%
  do(with(., cor.test(SynVar2,value, method="spearman") %>% broom::tidy() ))



cbind(microbiota_food_mcoa$SynVar,metadata_79ind, unifrac_all_pco$li[select_sample_mcoa,1:5]) %>%
  #merge(.,nutriscore_DI, by.y="Patient_ID", by.x="row.names") %>%
  select(SynVar2,A2,IBS.Severity.Score,A4, nutriscore_DI) %>%
  melt(id.vars="nutriscore_DI") %>% 
  group_by(variable) %>%
...
cbind(microbiota_food_mcoa$SynVar,metadata_79ind, infraspecies_all_pco$li[select_sample_mcoa,1:5]) %>%
  #merge(.,nutriscore_DI, by.y="Patient_ID", by.x="row.names") %>%
  select(SynVar2,A1,IBS.Severity.Score,A4, nutriscore_DI) %>%
  melt(id.vars="SynVar2") %>% 
  group_by(variable) %>%
  do(with(., cor.test(SynVar2,value, method="spearman") %>% broom::tidy() ))
...
data.frame(microbiota_food_mcoa$SynVar,metadata_79ind, unifrac_all_pco$li[select_sample_mcoa,1:5], infraspecies_all_pco$li[select_sample_mcoa,1:5]) %>%
  select(SynVar4,IBS.Severity.Score,A5,A4.1) %>%
  melt(id.vars="SynVar4") %>%
  ggplot(aes(x=SynVar4, y=value)) + geom_point() + geom_smooth(method = "lm") + facet_wrap(~variable, scale="free_y")
...
data.frame(microbiota_food_mcoa$SynVar,metadata_79ind, unifrac_all_pco$li[select_sample_mcoa,1:5], infraspecies_all_pco$li[select_sample_mcoa,1:5])  %>%
  ...
  melt(id.vars="SynVar4") %>%
  group_by(variable) %>%
  do(with(., cor.test(SynVar4,value, method="pearson") %>% broom::tidy() ))
