...
food_pco$li %>% row.names

infraspecies_pco$li %>% row.names


metadata_79ind =
IBSMicrobiota::IBSData$metadata %>% 
  filter(Visit == "V4", Sample_type =="Stool") %>%
  filter(Patient_ID %in% row.names(infraspecies_pco$li)) %>%
...
metadata_79ind %>% scale(scale=TRUE)


w.ktab = cbind(infraspecies_pco$li[1:5],food_pco$li[1:5], metadata_79ind %>% scale(scale=TRUE)) %>%
  ktab.data.frame(blocks = c(5,5,10), tabnames = c("microbiota","food","metadata"))


microbiota_food_mcoa <- mcoa(w.ktab,  scan = FALSE)
...
cbind(microbiota_food_mcoa$SynVar,metadata_79ind, food_pco$li[1:5]) %>%
  select(SynVar2,A2,IBS.Severity.Score,A4) %>%
...
cbind(microbiota_food_mcoa$SynVar,metadata_79ind, food_pco$li[1:5]) %>%
  ...
  melt(id.vars="SynVar2") %>% 
  group_by(variable) %>%
  do(with(., cor.test(SynVar2,value) %>% broom::tidy() ))


cbind(microbiota_food_mcoa$SynVar,metadata_79ind, infraspecies_pco$li[1:5]) %>%
  select(SynVar2,A1,IBS.Severity.Score,A4) %>%
  melt(id.vars="SynVar2") %>% 
  group_by(variable) %>%
  do(with(., cor.test(SynVar2,value) %>% broom::tidy() ))
