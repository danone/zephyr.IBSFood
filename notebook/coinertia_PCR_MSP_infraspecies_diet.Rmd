...
rbind(
  rbind(
    MSP_infraspecies_food_coi$lX[1:2] %>% dplyr::rename(A1=AxcX1, A2=AxcX2) %>% mutate(Set="Train set",Data="metagenomes"),
    MSP_infraspecies_food_coi_spesup$lisup[1:2] %>% dplyr::rename(A1=AxcX1, A2=AxcX2) %>% mutate(Set="Validation set",Data="metagenomes")),
  rbind(
    MSP_infraspecies_food_coi$lY[1:2] %>% dplyr::rename(A1=AxcY1, A2=AxcY2) %>% mutate(Set="Train set",Data="food"),
...
severity_modeling) %>%
  ...
  scale_color_discrete("Validation data") +
  scale_shape("Training data") +
  facet_wrap(~target, nrow = 1, scales = "free") + ylab("predicted (scaled)") + xlab("observed")
...
cowplot::plot_grid(
cowplot::plot_grid(p_partial_co_inertia+ cowplot::theme_cowplot(),  model_prediction_plot+ cowplot::theme_cowplot(), labels = c("A","C"), rel_widths = c(1,1.5)), 
cowplot::plot_grid(p_coinertia_metadata+ cowplot::theme_cowplot(),NULL, labels = c("B",""), rel_widths = c(9,1)), 
nrow=2)
