...
MSP_tax %>%
  filter(genus == "Streptococcus") #check thetaiotamicron
...
foreach(msp_name_target[1:3] , .combine = rbind) %dopar% {

merge(counts_rescaled, 
      MSP_genes_id_corresp %>% 
        select(msp_name,module_name,gene_name_v2) %>% 
        filter(msp_name %in% msp_name_target),
      by.x="genes.id", by.y="gene_name_v2") -> msp_counts
...
convert_MSP_to_infraspecies(MSP_abundance = MSP_abundance, clusters_msp_sample = clusters_msp$sample, msp_counts = msp_counts) -> infraspecies_abundance
