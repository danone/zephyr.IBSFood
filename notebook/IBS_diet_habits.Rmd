---
title: "IBS food and dietary habits"
output:
...
Let's focus on metadata only.

```{r}


IBSData$metadata %>% select(Patient_ID,Health,Gender,Age,BMI,H2,CH4,SSgroup,transit.days) %>% head(20)
...
food_group_levels =
food_group_levels %>%
  dplyr::rename(Food_items_swe = Livsmedelsnamn, Food_items_eng = Livsmedelsnamn_eng, Food_items_id = Livsmedelsnummer) %>%
  filter(used == "yes") %>%
  ...
  mutate(`Food groups lvl1` = `Food groups lvl1` %>% gsub("Animal-based","Animal-based",.))
...
food_swe %>%
  dplyr::rename(Food_items_swe = Livsmedelsnamn, Food_items_id = Livsmedelsnummer) %>%
...
food_group_levels %>%
  filter(`Food groups lvl1` %in% c("Others")  )
...
TAX = tax_table(food_group_levels %>%
              ...
              unique %>%
              na.omit() %>%
              filter(`Food groups lvl4` %in% rownames(food_otu)) %>% as.data.frame %>% arrange(`Food groups lvl4`) %>% tibble::column_to_rownames("Food groups lvl4")%>% as.matrix)
...
unifrac_pco  = ade4::dudi.pco(food_unifrac %>% ade4::quasieuclid(), scannf=F, nf=5)
...
merge(OTU %>% prop.table(2) ,TAX, by="row.names") %>%
  dplyr::rename(`Food groups lvl4` = Row.names) %>%
...
#to clean figure because there are doublons
food_to_remove = c("Eggs and egg dishes","Nuts and seeds", "Coffee and tea", "Legumes")
...
ggplot() + geom_point(aes(x=A1*2,y=A2*2, shape=Health, color=Gender), size=4) +
    ...
    geom_point(data=cor_food_tree_res %>% filter(abs(cor_A1)>0.3 | abs(cor_A2)>0.3 ), aes(x=cor_A1,y = cor_A2)) + ggrepel::geom_text_repel(data=cor_food_tree_res %>% filter(!Type %in% food_to_remove) %>% filter(!(Hierarchy=="Food groups lvl2"& Type=="substitutes for milk")) %>% filter(abs(cor_A1)>0.3 | abs(cor_A2)>0.3 ), aes(x=cor_A1,y = cor_A2,label=Type)) + 
    xlab(paste0("PCoA1 (",round((unifrac_pco$eig)/sum(unifrac_pco$eig)*100,1)[1], "%)")) +
  ylab(paste0("PCoA2 (",round((unifrac_pco$eig)/sum(unifrac_pco$eig)*100,1)[2],"%)")) -> food_tree_unifrac_pcoa_plot
...
unifrac_pco$li %>%
  ...
  scale_color_brewer("Quantiles") + ylab("PCoA1") -> Gender_pcoa_plot
...
unifrac_pco$li %>%
 ...
 ggplot() + 
  ...
  scale_color_brewer("Quantiles") + ylab("PCoA2") -> Health_pcoa_plot
...
unifrac_pco$li %>%
  merge(.,
   ...
   by.y="Patient_ID",
   by.x="row.names") %>%
  mutate(Health_severe = ifelse(SSgroup == "severe","severe","non-severe")) %>%
  select(A2,Health_severe, SSgroup) %>%
  #filter(SSgroup %in% c("control","severe")) %>%
  #na.omit() %>%
  with(.,wilcox.test(.$A2~.$Health_severe))
...
unifrac_pco$li %>%
  merge(.,
   ...
   by.y="Patient_ID",
   by.x="row.names") %>%
  mutate(Health_severe = ifelse(SSgroup == "severe","severe","non\nsevere")) %>%
  select(A2,Health_severe) %>%
  na.omit() %>%
 ggplot() + 
  geom_interval(data= . %>% group_by(Health_severe) %>% tidybayes::mean_qi(A2=A2*2 , .prob = c(.5, .66, .95, .99)) , aes(x=Health_severe, y=A2)) +
  #geom_jitter(aes(x=Gender, y=A1), width=0.1) +
  geom_point(data = . %>% group_by(Health_severe) %>% summarize(A2=median(A2*2)), aes(x=Health_severe, y=A2) , size=5, shape=18  ) + 
  scale_color_brewer("Quantiles") + ylab("PCoA2") -> Severity_pcoa_plot
...
# compute Meat/Plant ratio (not animal!!)
  merge(OTU %>% prop.table(2) ,TAX, by="row.names") %>%
    as.data.frame %>%
    dplyr::rename(`Food groups lvl4` = Row.names) %>%
    melt(id.vars=c("Food groups lvl1","Food groups lvl2","Food groups lvl3","Food groups lvl4")) %>%
    filter(!(`Food groups lvl2` %in% c("Dairy products","Eggs and egg dishes","Fish") )) %>%
    group_by(variable,`Food groups lvl1`) %>%
    summarize(Gram_prop = sum(value)) %>%
    
    dcast(variable~`Food groups lvl1`) %>%
    mutate(meat_plant_ratio = log2(`Animal-based`+0.001/`Plant-based`+0.001)) %>%
  ...
  merge(OTU %>% prop.table(2) ,TAX, by="row.names") %>%
    ...
    dplyr::rename(`Food groups lvl4` = Row.names) %>%
    melt(id.vars=c("Food groups lvl1","Food groups lvl2","Food groups lvl3","Food groups lvl4")) %>%
    filter(!(`Food groups lvl2` %in% c("Dairy products","Eggs and egg dishes") )) %>%
    group_by(variable,`Food groups lvl1`) %>%
    summarize(Gram_prop = sum(value)) %>%
    dcast(variable~`Food groups lvl1`) %>%
    mutate(meat_plant_ratio = log2(`Animal-based`+0.01/`Plant-based`+0.01)) %>%
    merge(.,unifrac_pco$li, by.x="variable", by.y="row.names") %>%
...
merge(OTU %>% prop.table(2) ,TAX, by="row.names") %>%
     ...
     dcast(variable~`Food groups lvl1`) %>%
     mutate(meat_plant_ratio = log2(`Animal-based`+0.01/`Plant-based`+0.01)) %>%
     merge(.,unifrac_pco$li, by.x="variable", by.y="row.names") %>% with(.,cor.test(.$meat_plant_ratio,.$A1, method="spearman" ))
...
food_group_levels %>%
  filter(`Food groups lvl1` == "Animal-based")
...
food_swe %>%
  ...
  dplyr::rename(Food_items_id = Livsmedelsnummer) %>%
...
food_data %>%
  group_by(Identity,Day,Meal) %>%
  summarize(n=sum(Gram)) %>%
  mutate(snack = ifelse(Meal %in% c("Breakfast","Dinner","Lunch"),"no","yes"  )) %>%
  ungroup() %>%
  group_by(Identity,Day,snack) %>%
  summarize(n=n()) %>%
  filter(snack=="yes") %>%
  ungroup() %>%
  group_by(Identity) %>%
  summarize(snack_per_day = mean(n)) -> snack_per_day
...
nutriscore_DI = sum(nutriscore per food x energy intake) / (total energy intake)
...
food_data %>%
  ...
  dplyr::rename(Foodstuffs = Foodstuffs.x) %>%
  group_by(Identity, `Food groups lvl1`, `Food groups lvl2`, `Food groups lvl3`,`Food groups lvl4`) %>%
  summarise(`Energy kJ` = sum(`Energy kJ`)) %>%
  na.omit() %>%
  dcast(Identity ~ `Food groups lvl4`, value.var = "Energy kJ",fill = 0 ) %>%
    tibble::column_to_rownames("Identity") %>%
  t() %>%
  #as_tibble %>%
  round() -> food_otu_EnergyKJ
...
food_otu_EnergyKJ %>% 
  reshape2::melt() %>%
  merge(nutriscore_food_group,.,by.x="Food groups lvl4", by="Var1") %>%
  mutate(nutriscore_weighted = nutriscore * value) %>%
  group_by(Var2) %>%
  summarize(nutriscore_DI = sum(nutriscore_weighted)/sum(value)) %>%
  dplyr::rename(Patient_ID = Var2) -> nutriscore_DI


merge(nutriscore_DI,unifrac_pco$li,by.x="Patient_ID",by.y="row.names") %>%
   merge(.,snack_per_day,by.x="Patient_ID",by.y="Identity") %>%
  with(.,cor.test( .$nutriscore_DI, .$A2, method="spearman" ))
  

merge(nutriscore_DI,unifrac_pco$li,by.x="Patient_ID",by.y="row.names") %>%
  merge(.,snack_per_day,by.x="Patient_ID",by.y="Identity") %>%
    ggplot() + geom_point(aes(x=A2,y=nutriscore_DI)) + 
  #geom_smooth(aes(x=A2,y=nutriscore_DI), method = "lm") +
  ylab("Nutriscore\nDiet index") + xlab("PCoA2") -> nutriscore_DI_plot

save(nutriscore_DI, file="nutriscore_DI.rda")


nutriscore_DI_plot

ggsave(filename = "nutriscore_DI_plot.pdf")
...
#### diet index comparisons between HC and IBS
```{r}

read.csv2("study_global_metadata.csv", row.names=1) %>%
  merge(nutriscore_DI, by="Patient_ID", all = FALSE) %>%
  ggplot() + geom_boxplot(aes(y=nutriscore_DI,x=Health))

read.csv2("study_global_metadata.csv", row.names=1) %>%
  merge(nutriscore_DI, by="Patient_ID", all = FALSE) %>%
  ggplot() + geom_boxplot(aes(y=nutriscore_DI,x=SSgroup))

read.csv2("study_global_metadata.csv", row.names=1) %>%
  merge(nutriscore_DI, by="Patient_ID", all = FALSE) %>%
  with(., wilcox.test(.$nutriscore_DI~.$Health))

read.csv2("study_global_metadata.csv", row.names=1) %>%
  merge(nutriscore_DI, by="Patient_ID", all = FALSE) %>%
  group_by(Health) %>%
  summarise(m=median(nutriscore_DI), q25=quantile(nutriscore_DI, 0.25), q75=quantile(nutriscore_DI, 0.75))
...
food_data %>%
  filter(Meal %in% c("Breakfast","Lunch","Dinner")) %>%
  merge(food_group_levels, by.x="Code",by.y="Food_items_id", all = FALSE) %>%
  select(-Foodstuffs.y) %>%
  dplyr::rename(Foodstuffs = Foodstuffs.x) %>%
  group_by(Identity, `Food groups lvl1`, `Food groups lvl2`, `Food groups lvl3`,`Food groups lvl4`) %>%
  ...
  t() %>%
  #as_tibble %>%
  round() -> food_otu_EnergyKJ_main



food_otu_EnergyKJ_main %>% 
  reshape2::melt() %>%
  merge(nutriscore_food_group,.,by.x="Food groups lvl4", by="Var1") %>%
  mutate(nutriscore_weighted = nutriscore * value) %>%
  group_by(Var2) %>%
  summarize(nutriscore_DI = sum(nutriscore_weighted)/sum(value)) %>%
  dplyr::rename(Patient_ID = Var2) -> nutriscore_DI_main


read.csv2("study_global_metadata.csv", row.names=1) %>%
  merge(nutriscore_DI_main, by="Patient_ID", all = FALSE) %>%
  with(., wilcox.test(.$nutriscore_DI~.$Health))
...
food_data %>%
  ...
  #facet_wrap(~Meal) +
  scale_fill_brewer("Nutriscore\nCategory",type="div", direction=-1)

food_data %>%
  select(Identity,Foodstuffs, Meal, Day) %>%
  merge(cbind(Foodstuffs=food_test[,1],res), by="Foodstuffs") %>%
  ...
  mutate(prop=n/sum(n)) %>%
  filter(res %in% c("A","E")) %>%
  ggplot() + geom_boxplot(aes(x=Meal,y=prop, fill=res)) + 
  scale_fill_brewer("Nutriscore\nCategory",type="qual", direction=-1) + 
  ylab("Proportion of food items\nper subject") + scale_y_continuous(labels=scales::percent)-> nutriscore_meal_plot

nutriscore_meal_plot
...
food_data %>%
  ...
  mutate(prop=n/sum(n)) %>%
  ggplot() + geom_boxplot(aes(x=Meal,y=prop, fill=res)) + 
  scale_fill_brewer("Nutriscore\nCategory",type="div", direction=-1) + facet_wrap(.~res, ncol= 1) +
  ylab("Proportion of food items\nper subject")
...
nutriscore_DI %>%
     merge(.,
          # IBSData$metadata %>% 
          #   filter(Sample_type=="Stool", Visit=="V4") %>% 
          #     select(Patient_ID,Health,IBS.Severity.Score,SSgroup,H2,CH4), 
          read.csv2("study_global_metadata.csv", row.names=1),
           by="Patient_ID") %>% 
  mutate(severe=ifelse(SSgroup=="severe","severe","non severe")) %>%
  with(.,wilcox.test(nutriscore_DI~severe))
...
nutriscore_DI %>%
  ...
  by.x="Patient_ID", by.y="Identity") %>%
  mutate(IBS.Severity.Score = ifelse(Health=="control",0,IBS.Severity.Score)) %>%
  # group_by(Meal,res) %>%
  # do(
  # 
  # 
  # with(., wilcox.test(nutriscore_DI ~ prop, data=., method="spearman")) %>% broom::tidy()
  # )

  #ggplot() + geom_point(aes(y=nutriscore_DI , x=prop)) + facet_grid(Meal~res, scale="free")
  filter(res %in% c("A","E")) %>%
  filter(Meal %in% c("Breakfast","Lunch","Dinner")) %>%
  filter(!is.na(SSgroup)) -> main_meal_quality

main_meal_quality %>%
  mutate(Health_severe = ifelse(SSgroup == "severe","severe","non\nsevere")) %>%
  filter(SSgroup %in% c("control","severe")) %>%
  group_by(res) %>%
  do(with(., wilcox.test(prop ~ SSgroup, data=., method="spearman") %>% broom::tidy() ))
  
  
  main_meal_quality %>% 
    ggplot() + geom_boxplot(aes(y=prop , x=res, fill=SSgroup)) + scale_y_log10() + ylab("Food items prevalence\nper meal") + xlab("Food nutrient-based quality") + 
  scale_x_discrete(label=c("A (High)","E (Low)")) +
  scale_fill_manual("", label = c("Healthy control","Mild IBS","Moderate IBS","Severe IBS"), values= c("#018571","#80cdc1","#dfc27d","#a6611a")) -> nutriscore_severity_plot

save(nutriscore_severity_plot, file="nutriscore_severity_plot.rda")

nutriscore_severity_plot



```

#### compute fold ratio between high quality and low quality
```{r}
...
#   mutate(Health_severe = ifelse(SSgroup == "severe","severe","non\nsevere")) %>%
#   filter(SSgroup %in% c("control","severe")) %>%
#   group_by(res,SSgroup) %>%
#   summarise(m=median(prop), q25=quantile(prop, 0.25), q75=quantile(prop, 0.75))
#   

main_meal_quality %>%
  mutate(Health_severe = ifelse(SSgroup == "severe","severe","non\nsevere")) %>%
  filter(SSgroup %in% c("control","severe")) %>%
  select(Patient_ID,SSgroup,Meal,res,prop) %>%
  reshape2::dcast(Patient_ID+SSgroup+Meal~res, value.var = "prop", fill=0) %>%
  mutate(ratio=E/A) %>%
  group_by(SSgroup) %>%
  summarise(m=median(1/ratio))
...
nutriscore_DI %>%
  ...
  #group_by(res) %>%
  #do(with(., wilcox.test(prop ~ SSgroup, data=., method="spearman") %>% broom::tidy() ))
  ggplot() + geom_boxplot(aes(y=prop , x=res, fill=SSgroup)) + scale_y_log10() + ylab("Food items prevalence\nper snack") + xlab("Food nutrient-based quality") + 
  scale_x_discrete(label=c("A (High)","E (Low)")) +
  scale_fill_manual("", label = c("Healthy control","Mild IBS","Moderate IBS","Severe IBS"), 
...
nutriscore_DI %>%
  ...
  filter(res %in% c("A","E")) %>%
  filter(!(Meal %in% c("Breakfast","Lunch","Dinner"))) %>%
  filter(!is.na(SSgroup)) -> snack_quality
...
diet_plot = cowplot::plot_grid(
  ...
  cowplot::plot_grid(
    ...
    cowplot::plot_grid(
      Gender_pcoa_plot+ cowplot::theme_cowplot(),
      Severity_pcoa_plot+ cowplot::theme_cowplot() + xlab("Symptoms severity"), nrow=2, labels=c("D","E")),
    rel_widths = c(4,1.2), labels=c("A","")),
  
  cowplot::plot_grid(
    meat_plant_pcoa_plot+ cowplot::theme_cowplot(),
    nutriscore_DI_plot+ ylab("FSA-NPS\nDiet Index") + cowplot::theme_cowplot(), 
    #nutriscore_severity_plot + cowplot::theme_cowplot() + theme(axis.text.x = element_text(angle = 45, hjust = 1)), 
    nutriscore_severity_plot + cowplot::theme_cowplot() ,
    rel_widths = c(1,1,1.3),nrow=1, labels=c("B","C","F")), nrow = 2, rel_heights = c(2,1))
...
diet_plot = cowplot::plot_grid(
  ...
  cowplot::plot_grid(
    ...
    cowplot::plot_grid(
      Gender_pcoa_plot+ cowplot::theme_cowplot(),
      Severity_pcoa_plot+ cowplot::theme_cowplot() + xlab("Symptoms severity"), nrow=2, labels=c("c","d")),
    rel_widths = c(4,1.2), labels=c("b","")),
  ...
  cowplot::plot_grid(
    ...
    #nutriscore_severity_plot + cowplot::theme_cowplot() + theme(axis.text.x = element_text(angle = 45, hjust = 1)), 
    nutriscore_severity_plot + cowplot::theme_cowplot() ,
    rel_widths = c(1,1,1.3),nrow=1, labels=c("e","f","g")), nrow = 2, rel_heights = c(2,1))


load("food_tree_plot.rda")

cowplot::plot_grid(
  food_tree_plot,
  diet_plot, nrow=2,labels=c("a","") )

ggsave("Figure1_Diet_PCoA_food_tree_plot_small.pdf")
...
#IBS vs healthy
readxl::read_excel(system.file("data-raw","MOSAIC food groups.xlsx", package = "IBSFood"), sheet = "Nutr & food groups") %>%
  filter(!is.na(kcal)) %>%
  select(1,2,10:58) %>%
  rename(Health = `Patient(1=patient)`) %>%
  mutate(Health = ifelse(Health=="1","Patient","Control")) %>%
 
  melt(id.vars=c("ID","Health")) %>%
  filter(variable != "kJ") %>%
  group_by(variable) %>%
  mutate(value=scale(value %>% as.numeric)) %>%
  do(t.test(.$value ~ .$Health, data=.) %>% broom::tidy()) %>%
  mutate(delta_Z = estimate1 - estimate2) %>%
  select(variable,p.value,delta_Z) %>%
  ggplot() + geom_bar(aes(x=variable,y=abs(delta_Z), fill=delta_Z>0, alpha=p.value<0.05 ), stat="identity") + 
  ylim(0,1) + coord_flip() + scale_fill_brewer("",type="qual", palette = "Set1", labels=c("Higher in IBS","Lower in IBS")) + 
  scale_alpha_discrete(range=c(0.4,1)) + theme_classic() + xlab("") + ylab("Absolute Z-score\n(mean difference)")

ggsave("FigureS1_nutrient_IBS.pdf")
...
readxl::read_excel(system.file("data-raw","MOSAIC food groups.xlsx", package = "IBSFood"), sheet = "Nutr & food groups") %>%
 ...
 merge(.,
     IBSData$metadata %>%
       select(Patient_ID, SSgroup) %>% unique(),
   by.y="Patient_ID",
   by.x="ID") %>%
  mutate(Health_severe = ifelse(SSgroup == "severe","severe","non\nsevere")) %>%

melt(id.vars=c("ID","Health_severe","Health", "SSgroup")) %>%
  na.omit() %>%
  mutate(Health_severe = Health_severe %>% as.character) %>%
  group_by(variable) %>%
  filter(variable != "kJ") %>%
  group_by(variable) %>%
  mutate(value=scale(value %>% as.numeric)) %>%
  filter(Health_severe %in% c("severe","non\nsevere") ) %>%
  mutate(Health_severe = Health_severe %>% as.character) %>%
  do(t.test(.$value ~ .$Health_severe, data=.) %>% broom::tidy()) %>%
  mutate(delta_Z = estimate1 - estimate2) %>%
  select(variable,p.value,delta_Z) %>%
  ggplot() + geom_bar(aes(x=variable,y=abs(delta_Z), fill=delta_Z>0, alpha=p.value<0.05 ), stat="identity") +
  ylim(0,1) + coord_flip() + scale_fill_brewer("", type="qual", palette = "Set1",labels=c("Higher in severe IBS","Lower in severe IBS")) +
  scale_alpha_discrete(range=c(0.4,1)) + theme_classic() + xlab("") + ylab("Absolute Z-score\n(mean difference)")


ggsave("FigureS2_nutrient_severe.pdf")
...
Axis 1 is linked with total of FODMAPS without Lactose (GOS & Fructans)
Axis 2 is negatively linked to Polyols amounts

```{r}
data("nutrients_data")

nutrients_data %>%
  select(ID,GOS,Fructans,Polyols,Lactose,`Total FODMAPs`) %>%
  merge(unifrac_pco$li,by.x="ID",by.y="row.names") %>%
  reshape2::melt(id.vars=c("A1","A2","A3","ID")) %>%
  dplyr::rename(fodmap_value=value,fodmap_variable=variable) %>%
  reshape2::melt(id.vars=c("ID","fodmap_value","fodmap_variable")) %>%
  group_by(fodmap_variable,variable) %>%
  do(r=with(.,cor.test(fodmap_value,value, method="spearman") %>% broom::tidy() )) %>%
  tidyr::unnest(r, .drop = TRUE) %>%
...
nutrients_data %>%
   select(Fructose,GOS,Fructans,Polyols,Lactose,`Excess fructose`,`Total FODMAPs`) %>%
  mutate(total=GOS+Fructans+Polyols+Lactose+`Excess fructose`)
